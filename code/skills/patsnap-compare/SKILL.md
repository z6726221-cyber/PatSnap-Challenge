...
12. **缺口收束**：无论资料是否充足，末尾必须有 `## 资料缺口 / 待核实项`。资料充足时写“本次已召回资料未发现额外缺口”，仍保留标题；资料不足时列缺口和建议补料。
13. **自检**：见 Verification。
...
- 推荐整体顺序：`## <报告标题>` → 一段摘要 → `## 核心结论` → 标准 markdown 对比表 → `## 建议销售话术` → `## 资料缺口 / 待核实项`。
- 若资料不足导致表格不完整，仍输出可用表格，但在表后单独列“资料缺口/建议补料”，不要让缺口散落在正文里。
- 即使资料看起来充足，也保留 `## 资料缺口 / 待核实项`，写明“本次已召回资料未发现额外缺口”；不要省略该标题。
