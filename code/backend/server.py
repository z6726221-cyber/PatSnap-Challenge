...
_MODE_SKILL = {
    ...
    "tech_explain": "patsnap-tech-explain",
}
...
def handle_chat(body: dict) -> dict:
    ...
    try:
        if explicit_case:
            ...
            if explicit_mode in ("promo", "comparison", "presales", "tech_explain"):
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _handle_upload(self):
        ...
        try:
            ...
            return self._send(400, {"error": str(e)})
        return self._send(200, {"upload": item, "message": "上传成功，已写入对应知识库；当前任务附件不会写入 live 黑盒检索目录"})
